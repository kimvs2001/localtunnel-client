...
module.exports = class Tunnel extends EventEmitter {
  ...
  _getInfo(body) {
    ...
    const { host, port: localPort, local_host, local_port } = this.opts;
    ...
    return {
      ...
      remote_ip: ip,
      remote_port: port,
      local_port: local_port || localPort,
      local_host,
      local_https,
      local_cert,
      local_key,
  ...
  _init(cb) {
    ...
    console.log('hi, zzzconnecting to server... opt.host:',opt.host);
    console.log('hi,zzz connecting to server... opt.subdomain:',opt.subdomain);  
    console.log('hi,zzz connecting to server... local_port,:',opt.local_port);
    (function getUrl() {
      console.log("getURL");
      axios
        .get(uri, params)
        ...
        .catch(err => {
          ...
          console.log(`Tunnel.js , tunnel server offline: ${err.message}, retry 1s`);
  ...
  _establish(info) {
    ...
    this.tunnelCluster.on('dead', () => {
      ...
      console.log('Tunnel.js , tunnel dead [total: %d]', tunnelCount);
      if (this.closed) {
        return;
      }
